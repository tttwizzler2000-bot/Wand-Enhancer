    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="border-b border-white/6 px-3.5 py-2.5">
                <SearchInput
                    value={query}
                    placeholder={_(msg`Search games`)}
            ...
            <div className="flex items-center gap-2 px-3.5 pb-1.5 pt-3.5">
                {icon ? (
                    <Icon
                        className={cn(
                            'size-3',
                            accent ? 'text-(--deck-accent)' : 'text-(--deck-fg-4)',
                        )}
                        name={icon}
                        stroke={2}
                    />
                ...
                {typeof count === 'number' ? (
                    <span className="font-mono text-[10px] text-(--deck-fg-4)">{count}</span>
                ) : null}
            </div>
            {children}
        </section>
...
const GameRow = ({ game, canLaunch, query, onPin, onPlay, onStop }: GameRowProps) => {
    ...
    return (
        <article
            className={cn(
                'mx-2 mb-1 flex items-center gap-2.5 rounded-[10px] border px-2.5 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]',
                game.running
                    ? 'border-[color-mix(in_oklab,var(--deck-accent)_24%,transparent)] bg-[color-mix(in_oklab,var(--deck-accent)_7%,transparent)]'
                    : 'border-white/7 bg-white/2.5',
            )}
        >
            <GameCover game={game} />
            <div className="min-w-0 flex-1">
                <h4 className="truncate text-[13.5px] font-semibold text-(--deck-fg)">
            ...
            <div className="flex shrink-0 gap-1">
                <IconButton
                    size="sm"
                    active={game.pinned}
                    label={game.pinned ? _(msg`Remove favorite`) : _(msg`Favorite game`)}
                    icon={game.pinned ? 'star-filled' : 'star'}
                    onClick={handlePin}
                />
                {game.running ? (
                    <IconButton
                    ...
                    <IconButton
                        size="sm"
                        disabled={!canLaunch || !game.gameId}
                        accent
                        label={_(msg`Play`)}
                        icon="play"
                        onClick={handlePlay}
...
function highlightTitle(title: string, query: string): ReactNode {
    ...
    return (
        <>
            {before}
            <span className="rounded-[3px] bg-[color-mix(in_oklab,var(--deck-accent)_25%,transparent)] px-0.5 text-(--deck-fg)">
                {match}
            </span>
            {after}
        </>

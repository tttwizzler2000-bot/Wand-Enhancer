...
const OPTIONAL_SERVICE_SPECS = [
    {
        ...
        methods: [
            'getUnavailableTitle',
            'searchUnavailableTitles',
            'getUnavailableTitlesByCorrelationIds',
        ],
    ...
    },
    {
        ...
        methods: [
            'getLatestLocalTrainerForGame',
            'getMostCompatibleTrainerForGame',
            'getTrainerById',
        ],
...
export function getInstalledAppsService(state, container, webpackRequire) {
    const ctor = findExportedConstructor(webpackRequire, (prototype) => {
        return (
            typeof prototype.refreshApps === 'function' && typeof prototype.watchGame === 'function'
        );
    });

    if (!ctor) {
        state.log('warn', 'Installed apps service constructor not found in webpack cache.');
        return null;
    }
...
export function getStoreRef(state, container, webpackRequire) {
    ...
    try {
        ...
        const storeState =
            typeof store.state?.getValue === 'function' ? store.state.getValue() : null;
        ...
        state.log('warn', 'Failed to resolve Store from container.', formatError(error));
        return null;
    }
}
...
function findFallbackService(state, container, warningKey, missingMessage, predicate) {
